...
JsDatePick.prototype.setConfiguration = function(aConf){
	...
	////RKR:add avialDaylist
	this.availDayList	= (aConf["availDayList"] != null) ? aConf["availDayList"] : null;
	this.halfDayList 	= (aConf["halfDayList"] != null) ? aConf["halfDayList"] : null;
	this.notAvailDayList 	= (aConf["notAvailDayList"] != null) ? aConf["notAvailDayList"] : null;
...
JsDatePick.prototype.populateMainBox = function(aMainBox){
	...
	while (oDay.getMonth() == cmpMonth){
		...
		//RKR:add special flag
		var specialDay_flag=false;
		var selected = false;
		...
		aDayDiv.onmouseup = function(){
			...
			*/
			if (parseInt(this.getAttribute("isSpecial")) == 1)
			{
				return;
			}

			if (parseInt(this.getAttribute("isSelectedBefore")) == 1)
			{
				//make this normal
				this.style.background="white";
				this.style.color="black";
				gRef.setDayUnSelection(this);
			} else {
				//make this as selected
				this.style.background="#0d6fdd";
		...
		//RKR
		if(this.availDayList && this.isAvailableDay(oDay)){
			aDayDiv.style.background = "#0ddd1c";
			aDayDiv.style.color = "white";
			aDayDiv.setAttribute("isSpecial", 1);
		}else if(this.notAvailDayList && this.isHalfDay(oDay)){
			aDayDiv.style.background = "#b30ddd";
			aDayDiv.style.color = "white";
			aDayDiv.setAttribute("isSpecial", 1);
		}else if(this.halfDayList && this.isNotAvailableDay(oDay)){
			aDayDiv.style.background = "#c0c0c0";
			aDayDiv.style.color = "white";
			aDayDiv.setAttribute("isSpecial", 1);
		}
...
JsDatePick.prototype.setDaySelection = function(anElement){
	...
	//RKR
	anElement.setAttribute("isSelectedBefore", 1);
	
};

JsDatePick.prototype.setDayUnSelection = function(anElement){
	var currentColorScheme = this.getCurrentColorScheme();
	
	//RKR
	anElement.setAttribute("isSelectedBefore", 0);
};
...
//RKR: check the day is avalilable day
JsDatePick.prototype.isAvailableDay = function(aDateObject){
	var num = this.availDayList.length, i, oneDay, find=false;
	
	for(i=0; i<num; i++)
	{
		oneDay=this.availDayList[i];
		if (aDateObject.getDate() == oneDay.getDate() &&
		aDateObject.getMonth() == oneDay.getMonth() &&
		aDateObject.getFullYear() == oneDay.getFullYear()){
			find=true;
			break;
		}
	}
...
JsDatePick.prototype.isHalfDay = function(aDateObject){
	var num = this.halfDayList.length, i, oneDay, find=false;
	
	for(i=0; i<num; i++)
	{
		oneDay=this.halfDayList[i];
		if (aDateObject.getDate() == oneDay.getDate() &&
		aDateObject.getMonth() == oneDay.getMonth() &&
		aDateObject.getFullYear() == oneDay.getFullYear()){
			find=true;
...
JsDatePick.prototype.isNotAvailableDay = function(aDateObject){
	var num = this.notAvailDayList.length, i, oneDay, find=false;
	
	for(i=0; i<num; i++)
	{
		oneDay=this.notAvailDayList[i];
		if (aDateObject.getDate() == oneDay.getDate() &&
		aDateObject.getMonth() == oneDay.getMonth() &&
		aDateObject.getFullYear() == oneDay.getFullYear()){
			find=true;
